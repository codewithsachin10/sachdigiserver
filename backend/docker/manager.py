...
from backend.database import get_all_projects
...
def find_next_free_port() -> int:
    min_port = int(os.environ.get("PORT_RANGE_MIN", "7001"))
    max_port = int(os.environ.get("PORT_RANGE_MAX", "8100"))
    ...
    for port in range(min_port, max_port + 1):
        if port not in assigned_ports and not is_port_in_use(port):
            return port
    raise RuntimeError(f"No free ports left in configured range {min_port}-{max_port}!")
...
def run_container(image_name: str, container_name: str, host_port: int, internal_port: int, env_vars: dict = None) -> str:
    client = get_docker_client()
    ...
    # Enforce strict 512MB RAM and 0.5 CPU core limit for stability on 2GB server
    try:
        container_kwargs = {
            "image": image_name,
            "name": container_name,
